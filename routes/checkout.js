  ...
  try {
    const result = db.transaction(() => {
      ...
      for (const item of items) {
        if (!item.in_stock) {
          throw new Error(`${item.name_en} is no longer available.`);
        }

        if (item.track_stock && item.stock_deduction > Number(item.stock_qty || 0)) {
          const unit = item.type === 'local' ? 'ml' : 'units';
          throw new Error(
            `Not enough stock for ${item.name_en}. Need ${item.stock_deduction} ${unit}.`
          );
        }
      }
      ...
      const insertItem = db.prepare(`
        INSERT INTO order_items
          (order_id, product_id, product_name, quantity, size_ml, stock_deduction, price)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `);

      const movement = db.prepare(`
        INSERT INTO inventory_movements
          (product_id, movement_type, quantity, reference_type, reference_id, note, staff_name)
      ...
      for (const item of items) {
        ...
        if (item.track_stock) {
          ...
          movement.run(
            ...
            -item.stock_deduction,
            orderId,
            `${orderNumber}${item.size_ml ? ' · ' + item.size_ml + 'ml' : ''}`
          );
        }
      }
    ...
    req.flash(
      'error',
      lang === 'ar'
        ? 'تعذر إتمام الطلب بسبب المخزون. راجع السلة وحاول مجدداً.'
        : error.message
    );
